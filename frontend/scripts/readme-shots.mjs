// Curated screenshots for the README, all in the Day (light) theme. Usage: node scripts/readme-shots.mjs [baseUrl]
...
const light = async (opts) => {
  const ctx = await b.newContext(opts)
  await ctx.addInitScript(() => localStorage.setItem('hos.theme', 'light'))
  return ctx.newPage()
}

const desk = await light({ viewport: { width: 1440, height: 1000 }, deviceScaleFactor: 1.5 })
...
const phone = await light({ viewport: { width: 375, height: 812 }, deviceScaleFactor: 2, isMobile: true, hasTouch: true })
